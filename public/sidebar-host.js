(()=>{
 ...
 window.addEventListener('message',e=>{if(e.source!==parent||e.origin!==parent.location.origin)return;
  if(e.data?.type==='live-delivery-status'){const send=document.querySelector('#chat-send');if(send)send.title=e.data.error||'Send';}
 ...
 window.addEventListener('DOMContentLoaded',()=>{
  ...
  const configure=()=>{attach.disabled=false;attach.title='Add screenshots';document.querySelector('#chat-send').title='Send'};
