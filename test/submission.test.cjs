...
const png='data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+aE2sAAAAASUVORK5CYII=';
...
test('preserves edited order, repeated image references, numbering gaps and pasted quote text',()=>{
 ...
 assert.deepEqual(prompt.content.map(p=>p.type),['text','image','text','quote','text','image']);assert.equal(prompt.transcript,input.text);assert.equal(prompt.content[3].text,n.text);assert.equal(files.length,1);assert.equal(files[0].filename,'Image 3.png');assert.equal(prompt.content[1].attachmentId,prompt.content[5].attachmentId);assert.equal(prompt.attachments[0].width,1);
});
