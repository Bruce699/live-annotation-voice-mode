...
async function main(){
 ...
 if(command==='start'){
  ...
  let instance;try{instance=createServer({directory,project,preview,webhook:opts.receiver,webhookToken:opts['receiver-token-env']?process.env[opts['receiver-token-env']]:null})}catch(error){proxy?.close();throw error}
  if(opts['receiver-token-env']&&!process.env[opts['receiver-token-env']])throw Error('Receiver token environment variable is empty');
  ...
  await fs.mkdir(directory,{recursive:true,mode:0o700});const service={url:'http://localhost:'+port,token:instance.token,pid:process.pid,project,preview};await fs.writeFile(file,JSON.stringify(service),{mode:0o600});
  console.log('Live Annotation: '+service.url);if(project)console.log('Project: '+project.name+' · '+project.url+'\nData directory: '+directory);console.log(opts.receiver?'Receiver connected; submissions send immediately.':'Waiting for an agent receiver. Run: node cli.cjs receive --directory '+JSON.stringify(directory));
  instance.ready().catch(console.error);
  const stop=async()=>{instance.voice.close();instance.server.close();proxy?.close();try{const saved=JSON.parse(await fs.readFile(file));if(saved.token===service.token)await fs.unlink(file)}catch{}setTimeout(()=>process.exit(0),300).unref()};process.on('SIGINT',stop);process.on('SIGTERM',stop);return;
 }
